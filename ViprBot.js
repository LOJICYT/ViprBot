...
client.on('ready', function(){
    var ms = 3000    ;
    var setGame = ['v!help','v!help|v!inv','v!help|ViprBot','v!help | v!support','v!help|Dev|Revx','v!help|Dev|EipP'];
    var i = -1;
    var j = 0;
    setInterval(function (){
        ...
        i = i+j;
        client.user.setGame(setGame[i],`http://www.twitch.tv/revx_yt`);
    }, ms);
...
***https://discord.gg/jSR3WsQ***
<@!378953334251454475> : ***اذا واجهت مشكله واذا عندك استفسار***
<@!312244272000663564>
`);
...
:zap: <@!382588507115225099> :zap: 



:white_small_square: ***ViprBot سريع وحمايه قويه ومميزات كثيره وجديده***
:white_small_square: ***V!ar للاوامر العربيه***
:white_small_square: ***لاضافه البوت لسيرفرك***
:white_small_square: ️**https://goo.gl/fHTtQd**



:black_small_square:️ ***ViprBot Fast and powerful protection and many new features***
:black_small_square:️ ***V!en For English orders***
...
client.on('message', message => {
let args = message.content.split(' ').slice(1).join(' ');
if (message.content.startsWith('v!bcall')){
...
client.on('message', message => {
let args = message.content.split(' ').slice(1).join(' ');
if (message.content.startsWith('v!bc-user')){
 if(!message.author.id === 'ايدي صاحب البوت') return;
message.channel.sendMessage('تم , جار أرسال الرسالة')
client.users.forEach(m =>{
m.sendMessage(args)
})
...
client.on('message' , message => {
  if (message.author.bot) return;
    if(message.content.startsWith (prefix  + 'v!inv')) {
const embed = new Discord.RichEmbed()
.setColor("BLUE")
.setAuthor(message.author.username, message.author.avatarURL)
.setTitle(':arrow_right: اضغط هنا رابط البوت')
.setURL('http://curs.io/K3okpWRl')
message.channel.sendEmbed(embed);
}
});
